...
int USBMgr::listDevices( std::vector<PS3EYECam::PS3EYERef>& list )
{
    ...
    while ((dev = devs[i++]) != NULL) 
    {
		...
		if(desc.idVendor == PS3EYECam::VENDOR_ID && desc.idProduct == PS3EYECam::PRODUCT_ID)
		{
            ...
            ofLog(OF_LOG_VERBOSE, "iMan:iProd:iSer: %d:%d:%d%d\n",
                  desc.iManufacturer,
                  desc.iProduct,
                  desc.iSerialNumber,
                  desc.bcdDevice);
...
PS3EYECam::PS3EYECam(libusb_device *device)
{
   ...
   // ofLogVerbose() << "deviceAddress: " << ofToString(deviceAddress);
    //ostringstream cameraInfo;
    //cameraInfo << "device_descriptor.bcdDevice: " << device_descriptor.bcdDevice;
    //cout << "cameraInfo: " << cameraInfo.str() << endl;
